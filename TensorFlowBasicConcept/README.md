...
## Optimizer
- 损失函数
    - 损失函数是评估模型参数和特定输入时，表达模型输出的推理值与真实值之间不一致程度的函数
    - 常见的损失函数有平方损失函数，交叉熵算是函数和指数损失函数
    
- 经验风险
    - 使用损失函数对所有训练样本求损失值，再累加求平均可得到模型的经验风险。
    - 如果过渡追求训练数据上的地损失值，就会遇到过拟合问题，这时候模型的泛化能力就会变差
    - 模型训练的目标是不断最小化经验风险
    
- 正则化项和惩罚项
    - 为了降低过渡训练可能造成的过拟合风险，引入专门用来度量模型复杂度的正则化项和惩罚项
    - 常用的正则化项有L0,L1和L2范数
    - 将模型最优化的目标替换为鲁棒性更好的结构风险最小化
    - 模型训练的本质就是在最小化结构风险的同事取得最优的模型参数
    
- 优化算法
    - 求解最优化问题的算法称为优化算法，通常采用迭代方式实现
    - 不同优化算法的采用的迭代策略
        - 使用目标函数的一阶导数，如梯度下降法
        - 使用目标函数的二阶导数，如牛顿法
        - 使用前几轮迭代的信息，如Adam
